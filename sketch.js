...
function setup()
{
  ...
  elephant = createSprite(100,100,100,30);
  elephant.addImage(elephant_IMG);
  elephant.scale = 0.5;
...
function draw() 
{
  ...
  if(score != 3 && Hercule.isTouching(abovePoacher3) )
  {
    destroyAllThingsSCOREWISE();
    if(score ===  3)
    {
...
function final_score()
{
  ...
  text("YOU WON THE GAME!!", 300, -100);
  text("Press the 'e' key!!", 300, -75);
...
function destroyAllThings()
{
  ...
  textSize(18);
  fill("white");
  text("You Died. Reload The Page to Play Again", 150, Hercule.y + 100);
}

function destroyAllThingsSCOREWISE()
{
  Hercule.visible = false;
  poacher1.visible = false;
  poacher2.visible = false;
  ...
  textSize(18);
  fill("white");
  text("You do not have the required amount of seeds to get to the next level.", 25, Hercule.y - 100);
  text("Reload the Page to play again.", 150, Hercule.y - 75); 
}